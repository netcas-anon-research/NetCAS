    ...
    for jobnum in "${jobnums[@]}"; do
      for iter in $(seq 1 $iterations); do
        ...
        fio_cmd="fio --name=test --filename=$device --rw=randread --bs=$block_size --direct=1 \
                 ...
                 --runtime=600 --group_reporting --output-format=json"

        # fio_cmd="fio --name=test --filename=/dev/nvme2n1 --rw=randread --bs=4k --direct=1 \
        #          --ioengine=sync --size=1G --time_based --numjobs=64 --cpus_allowed=0-55 --cpus_allowed_policy=split \
